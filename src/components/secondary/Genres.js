  ...
  return (
    <div >
       ...
       <div><h1 style={{color:'rgb(255,0,70)'}} >Genres:-</h1>
         ...
         {genres?genres.slice(0,18).map(genre=><div className='genres' key={genre.name}  style={{textAlign:'center',justifyContent:'center',display:'flex',alignItems:'center',width:'105px',cursor:'pointer'}} id={genre.name} onClick={()=>{navigate(`/Genre/movie/${genre.name}/${genre.id}`)}}>{genre.name}</div>):''}
         </div>
